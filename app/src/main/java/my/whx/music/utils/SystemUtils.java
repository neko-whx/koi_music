...
public class SystemUtils {
    ...
    public static boolean isFlyme() {
        String flymeFlag = getSystemProperty();
        if (TextUtils.isEmpty(flymeFlag)) return false;
        assert flymeFlag != null;
    ...
    private static String getSystemProperty() {
        try {
            ...
            @SuppressLint("PrivateApi") Method getMethod = classType.getDeclaredMethod("get", String.class);
            return (String) getMethod.invoke(classType, "ro.build.display.id");
        } catch (Throwable th) {
            th.printStackTrace();
        }
        return null;
